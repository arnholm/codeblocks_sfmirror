...
const wxString Compiler::FilePathWithSpaces = "[][{}()[:blank:]#%$~[:alnum:]!&_:+/\\.-]+";
    ...
    m_Mirrored(other.m_Mirrored)
{
    ...
    m_ID = now.Format("%c", wxDateTime::CET);
    MakeValidID();
...
void Compiler::SaveSettings(const wxString& baseKey)
{
    ...
    tmp.Printf("%s/set%3.3d", baseKey.c_str(), CompilerFactory::GetCompilerIndex(this) + 1);
    cfg->DeleteSubPath(tmp);

    tmp.Printf("%s/%s", baseKey.c_str(), m_ID.c_str());
...
void Compiler::LoadSettings(const wxString& baseKey)
{
    ...
    SetIncludeDirs        (MakeUniqueArray(GetArrayFromString(cfg->Read(tmp + "/include_dirs",     wxString())), true));
    SetResourceIncludeDirs(MakeUniqueArray(GetArrayFromString(cfg->Read(tmp + "/res_include_dirs", wxString())), true));
    SetLibDirs            (MakeUniqueArray(GetArrayFromString(cfg->Read(tmp + "/library_dirs",     wxString())), true));
    ...
    for (int i = 0; i < ctCount; ++i)
    {
        ...
        for (size_t n = 0; n < keys.size(); ++n)
        {
            ...
            if (keys[n].Mid(4).ToULong(&index)) // skip 'tool'
            {
                ...
                wxString key        = wxString::Format("%s/macros/%s/tool%lu/", tmp.c_str(), CommandTypeDescriptions[i].c_str(), index);
    ...
    for (size_t i = 0; i < keys.GetCount(); ++i)
    {
        ...
        group.Printf("%s/regex/re%3.3ld", tmp.c_str(), index);
...
void Compiler::LoadDefaultOptions(const wxString& name, int recursion)
{
    ...
    ConfigManager* cfg = Manager::Get()->GetConfigManager("compiler");
    wxString cmpKey;
    cmpKey.Printf("%s/set%3.3d", baseKey.c_str(), CompilerFactory::GetCompilerIndex(this) + 1);
    if (!cfg->Exists(cmpKey + "/name"))
        cmpKey.Printf("%s/%s", baseKey.c_str(), m_ID.c_str());
    if (!cfg->Exists(cmpKey + "/name"))
