...
namespace
{
    PluginRegistrant<Autosave> reg("Autosave");
}
...
{
    if(!Manager::LoadResource("autosave.zip"))
    {
        NotifyMissingFile("autosave.zip");
    }
...
void Autosave::Start()
{
    ...
    if(cfg->ReadBool("do_project"))
        timer1->Start(60 * 1000 * cfg->ReadInt("project_mins"));
    else
        timer1->Stop();

    if(cfg->ReadBool("do_sources"))
        timer2->Start(60 * 1000 * cfg->ReadInt("source_mins"));
    else
        timer2->Stop();
}
...
void Autosave::SaveProject(cbProject *p, int method)
{
    ...
    switch(method)
    {
        case 0:
        {
            if(p->GetModified())
            {
                if(::wxRenameFile(p->GetFilename(), p->GetFilename() + ".bak"))
                    if(p->Save())
                    {
            ...
            if(::wxRenameFile(filename, filename + ".bak"))
                p->SaveLayout();
            break;
        }
        case 1:
        ...
        case 4: // doesn't really make sense to keep so many versions of a project file
        {
            ...
            if(p->GetModified())
            {
                ...
                if(loader.Save(p->GetFilename() + ".save"))
                {
                    CodeBlocksEvent e(cbEVT_PROJECT_SAVE, 0, p);
                    plm->NotifyPlugins(e);
                }
...
void Autosave::OnTimer(wxTimerEvent& e)
{
    if(e.GetId() == 10000)
    {
        ...
        const int method = cfg->ReadInt("method");
        const bool allProjects = cfg->ReadBool("all_projects", true);
        const bool doWorkspace = cfg->ReadBool("do_workspace", true);
        ...
        if(pm)// && pm->GetActiveProject())
        {
            ...
            if (doWorkspace && workspace && workspace->GetModified())
            {
                ...
                {
                    case 0:
                        if(::wxRenameFile(workspace->GetFilename(), workspace->GetFilename() + ".bak"))
                            workspace->Save();
                        break;
                    case 1:
                        workspace->Save();
                    ...
                    case 4:
                    {
                        ...
                        loader.Save(workspace->GetTitle(), workspace->GetFilename() + ".save");
    ...
    {
        int method = Manager::Get()->GetConfigManager("autosave")->ReadInt("method");
            ...
            for(int i = 0; i < em->GetEditorsCount(); ++i)
            {
                ...
                if(ed && ed->GetModified())
                {
                    ...
                    switch(method)
                    {
                        case 0:
                        {
                            if(::wxRenameFile(fn.GetFullPath(), fn.GetFullPath() + ".bak"))
                        ...
                        case 2:
                        {
                            cbSaveToFile(fn.GetFullPath() + ".save",
                        ...
                        case 4:
                        {
                            ...
                            {
                                tmp1.Printf("%s/%s.%u.%s", backupDir.c_str(), fn.GetName().c_str(), revisions,   fn.GetExt().c_str());
                                tmp2.Printf("%s/%s.%u.%s", backupDir.c_str(), fn.GetName().c_str(), revisions+1, fn.GetExt().c_str());
                            ...
                            tmp1.Printf("%s/%s.1.%s", backupDir.c_str(), fn.GetName().c_str(), fn.GetExt().c_str());
...
{
    wxXmlResource::Get()->LoadPanel(this, parent, "dlgAutosave");
...
void AutosaveConfigDlg::LoadSettings()
{
    ...
    XRCCTRL(*this, "all_projects", wxCheckBox)->SetValue(cfg->ReadBool("all_projects", true));
    int minsProjects = std::max(cfg->ReadInt("project_mins", 1), 1);
    int minsSources = std::max(cfg->ReadInt("source_mins", 1), 1);
    XRCCTRL(*this, "project_mins", wxTextCtrl)->SetValue(wxString::Format("%d", minsProjects));
    XRCCTRL(*this, "source_mins", wxTextCtrl)->SetValue(wxString::Format("%d", minsSources));
    ...
    XRCCTRL(*this, "method", wxChoice)->SetSelection(cfg->ReadInt("method", 2));
}
...
void AutosaveConfigDlg::SaveSettings()
{
    ...
    cfg->Write("do_project", (bool) XRCCTRL(*this, "do_project", wxCheckBox)->GetValue());
    cfg->Write("do_sources", (bool) XRCCTRL(*this, "do_sources", wxCheckBox)->GetValue());
    cfg->Write("do_workspace", (bool) XRCCTRL(*this, "do_workspace", wxCheckBox)->GetValue());
    cfg->Write("all_projects", (bool) XRCCTRL(*this, "all_projects", wxCheckBox)->GetValue());
