    ...
    m_Documentation(dh)
{
    ...
    XRCCTRL(*this, "spnParsersNum",            wxSpinCtrl)->SetValue(cfg->ReadInt("/max_parsers", 5));
    ...
    XRCCTRL(*this, "chkCCDebugLogging",        wxCheckBox)->SetValue(cfg->ReadBool("/CCDebugLogging",   false));
...
void CCOptionsDlg::OnApply()
{
    ...
    cfg->Write("/parser_per_workspace",          (bool) XRCCTRL(*this, "rdoOneParserPerWorkspace", wxRadioButton)->GetValue());
    cfg->Write("/max_parsers",                   (int)  XRCCTRL(*this, "spnParsersNum",            wxSpinCtrl)->GetValue());
    ...
    cfg->Write("/CCDebugLogging",    XRCCTRL(*this, "chkCCDebugLogging",  wxCheckBox)->GetValue());
    ...
    cfg->Write("/documentation_helper_background_color", (wxColour) XRCCTRL(*this, "cpDocBgColor",   wxColourPickerCtrl)->GetColour());
    cfg->Write("/documentation_helper_text_color",       (wxColour) XRCCTRL(*this, "cpDocTextColor", wxColourPickerCtrl)->GetColour());
    cfg->Write("/documentation_helper_link_color",       (wxColour) XRCCTRL(*this, "cpDocLinkColor", wxColourPickerCtrl)->GetColour());
    // -----------------------------------------------------------------------
...
void CCOptionsDlg::OnUpdateUI(cb_unused wxUpdateUIEvent& event)
{
    ...
    bool en = ccmcfg->ReadBool("/code_completion", true);
    ...
    en = ccmcfg->ReadBool("/code_completion", true);
...
bool CCOptionsDlg::ValidateReplacementToken(wxString& from, wxString& to)
{
    ...
    wxRegEx re("[A-Za-z_]+[0-9]*[A-Za-z_]*");
    if (!re.Matches(from))
    {
