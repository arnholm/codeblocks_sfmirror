...
// ----------------------------------------------------------------------------
{
    ...
    // use --compile-commands-dir=<path> to restrict compilation database search to <path>
    wxString projectName = pProject->GetFilename(); // (ph 26/08/19)
    wxString ccJsonDir = FindCompileCommandsJson(projectName);
    if (ccJsonDir.empty())
    {
        wxString editorProjectDotCBP = pProject->GetFilename();
        ccJsonDir = wxPathOnly(editorProjectDotCBP) + "\\compile_commands.json";
        if (not platform::windows) ccJsonDir.Replace("\\","/");
    }
    ccJsonDir = wxPathOnly(ccJsonDir);
    ccJsonDir.Prepend(" --compile-commands-dir=");
    command += ccJsonDir;
...
// ----------------------------------------------------------------------------
{
    ...
    // Don't bother parsing if file has no 'owner' project
    // Parsing just returns massive erroneous errors
    // These are files for which we cannot find any compile info
    // eg., #include files for system or lib headers
    //?if (not FindFilesOwningProject(infilename)) // (ph 26/08/22)
    //?    return false;
...
// ----------------------------------------------------------------------------
{
    wxUnusedVar(resultArray);
    wxUnusedVar(pProject);
...
// Returns the full path to the nearest compile_commands.json found by walking
// upward from the directory containing sourceFilename.
// Returns wxEmptyString if none is found.
// ----------------------------------------------------------------------------
wxString ProcessLanguageClient::FindCompileCommandsJson(const wxString& sourceFilename)
// ----------------------------------------------------------------------------
{
    wxFileName current(sourceFilename);

    // Treat the supplied filename as a file, begin search in its containing folder.
    current.SetFullName(wxEmptyString);
    current.Normalize(wxPATH_NORM_ABSOLUTE | wxPATH_NORM_DOTS);

    while (true)
    {
        wxFileName candidate(current);
        candidate.SetFullName("compile_commands.json");

        if (candidate.FileExists())
            return candidate.GetFullPath();

        // At a filesystem root, RemoveLastDir() can no longer make progress.
        const wxString before = current.GetPath();
        if (current.GetDirCount() == 0)
            break;
        current.RemoveLastDir();
        const wxString after = current.GetPath();
...
// Overwrites the existing destination file's contents with sourcePath's contents.
// Returns true only if the entire source was copied successfully.
//
// Note: another process must still permit this process to open the files with
// the requested sharing/access modes.
// ----------------------------------------------------------------------------
bool ProcessLanguageClient::CopyFileContents(const wxString& sourcePath,
                      const wxString& destinationPath, wxString* errorMessage)
// ----------------------------------------------------------------------------
{
    wxUnusedVar(errorMessage);

    json ccjson = json::array();
    if ( wxFileExists(sourcePath))    //(christo 2024/06/26)
    {
        std::fstream jsonFile;           //("out.json", std::ofstream::in | std::ofstream::out); //(christo 2024/06/26)
        jsonFile.open (sourcePath.ToStdString(), std::ofstream::in);
        ...
        jsonFile >> ccjson; //read file json object
        jsonFile.close();
    }//endif wxFileExists
    ...
    if (ccjson.size())
    {
        ...
        jsonFile.open (destinationPath.ToStdString(), std::ofstream::out | std::ofstream::trunc);
        if (not jsonFile.is_open())
        {
        ...
        {
            // update proxyProject compile_commands.json file
            jsonFile << ccjson; //write file json object
            jsonFile.close();
        }
    }
    return true;
...
// ----------------------------------------------------------------------------
cbProject* ProcessLanguageClient::FindFilesOwningProject(const wxString& filename)
// ----------------------------------------------------------------------------
{
	// Find the files owning/parent project
    // External header files outside workspace projects are not owned.
    ...
    wxFileName wanted(filename);
    wanted.Normalize(wxPATH_NORM_ABSOLUTE | wxPATH_NORM_DOTS);

    const ProjectsArray* const projects = projectManager->GetProjects();
    if (!projects)
        return nullptr;

    for (cbProject* project : *projects)
    {
        if (!project)
            continue;

        // GetFilesList() is Code::Blocks' list of files assigned to this project.
        for (ProjectFile* projectFile : project->GetFilesList())
        {
            if (!projectFile)
                continue;

            wxFileName candidate(projectFile->file.GetFullPath());
            candidate.Normalize(wxPATH_NORM_ABSOLUTE | wxPATH_NORM_DOTS);

#ifdef __WXMSW__
            // Windows paths are normally case-insensitive.
            if (candidate.GetFullPath().CmpNoCase(wanted.GetFullPath()) == 0)
#else
            if (candidate.GetFullPath() == wanted.GetFullPath())
#endif
            {
                return project;
            }
...
// ----------------------------------------------------------------------------
void ProcessLanguageClient::UpdateCompilationDatabase(cbProject* pProjectIn, wxString filename)
// ----------------------------------------------------------------------------
{
    // Update the Projects compile_commands.json database.
    ...
    if (pProject == m_pParser->GetParseManager()->GetProxyProject() )
    {
        ...
        // files of non-active projects, this function gathers compile info
        // from the files owning project (ie.,files parent project) because
        // ~ProxyProject~ is a dummy unopened project and has no appropriate
        // compile information.

        // clangd attempts to find another translation unit that somewhat
        // matches a files' name and uses its compile parameters.
        // Doing that can cause unreasonable/erroneous error messages.
        // We can make the parameters explicit by reading the compile parameters
        // from the files parent project, if one exists even if the project is inactive.
        // This is especially inportant when a file is opened from a dependency of the
        // current active project (eg., an inactive lib project or an inactive project
        // in the same workspace).

        // clangd starts searching for the database within the directory
        // containing the file, then searching backup (up) the tree for the file
        // 'compile_commands.json' created at the end of this function.
        // Note: this function is run before a file is handed off to clangd.

        // In this function we switch back and forth from the proxyProject pointer
        // to the actual project that owns the file, if any,
        // by setting var 'isProxyProject' to true allowing to choose data from
        // either the files' owning project or the proxy project.
        // The proxy project can be set to have compile info for files that
        // have no association with projects (future to do).
        // The 'pProject' pointer may be modified here, but its just a copy not
        // the "real deal".

        isProxyProject = true;

        // The fake ~ProxyProject~ is at (windows: appdata/roaming/codeblocks)
        // or the .config for CodeBlocks on Linux.
        // wxString userDataFolder = ConfigManager::GetConfigFolder();

        //  **Debugging**
        //cbProject* pProxyProject = pProject;
