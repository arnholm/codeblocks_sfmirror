...
struct RemoteDebugging
{
	...
	bool IsOk() const
	{
		return connType == Serial
				? (not serialPort.IsEmpty() && !serialBaud.IsEmpty())
				: (not ip.IsEmpty() && !ipPort.IsEmpty());
	}
	...
	void MergeWith(const RemoteDebugging& other)
	{
		...
		if (not additionalCmds.IsEmpty() && !other.additionalCmds.IsEmpty())
			additionalCmds += _T('\n');
		if (not other.additionalCmds.IsEmpty())
			additionalCmds += other.additionalCmds;

		if (not additionalCmdsBefore.IsEmpty() && !other.additionalCmdsBefore.IsEmpty())
			additionalCmdsBefore += _T('\n');
		if (not other.additionalCmdsBefore.IsEmpty())
			additionalCmdsBefore += other.additionalCmdsBefore;

		skipLDpath = other.skipLDpath;
		extendedRemote = other.extendedRemote;

		if (not additionalShellCmdsAfter.IsEmpty() && !other.additionalShellCmdsAfter.IsEmpty())
			additionalShellCmdsAfter += _T('\n');
		if (not other.additionalShellCmdsAfter.IsEmpty())
			additionalShellCmdsAfter += other.additionalShellCmdsAfter;

		if (not additionalShellCmdsBefore.IsEmpty() && !other.additionalShellCmdsBefore.IsEmpty())
			additionalShellCmdsBefore += _T('\n');
		if (not other.additionalShellCmdsBefore.IsEmpty())
